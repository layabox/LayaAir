(function (exports, Laya) {
	...
	class AdvImage extends Image {
	    ...
	    revertAdvsData() {
	        if (this.advsListArr[this._playIndex]) {
	            ...
	            if (Laya.Browser.onLimixiu) {
	                ...
	                if (advsObj) {
	                    if (Laya.ILaya.Browser.onLimixiu && window.GameStatusInfo.gameId == advsObj.gameid) {
	                        this.onLunbo();
	                    }
	                    else {
	    ...
	    isSupportJump() {
	        ...
	        else if (Laya.Browser.onLimixiu) {
	        ...
	        if (Laya.Browser.onLimixiu) {
	            if (!advsObj.isLunBo) {
	                if (!advsObj.isLunBo) {
	                    ...
	                    Laya.LocalStorage.setJSON("gameObj", gameAdvsObj);
	...
	class ScrollBar extends UIComponent {
	    constructor(skin = null) {
	        ...
	        this._showButtons = Laya.UIConfig.showButtons;
	        this._scrollSize = 1;
	        ...
	        this._touchScrollEnable = Laya.UIConfig.touchScrollEnable;
	        this._mouseWheelEnable = Laya.UIConfig.mouseWheelEnable;
	        this.skin = skin;
	        this.max = 1;
	    }
	    destroy(destroyChild = true) {
	...
	class DialogManager extends Laya.Sprite {
	    constructor() {
	        ...
	        if (Laya.UIConfig.closeDialogOnSide)
	    ...
	    _onResize(e = null) {
	        var Laya$1 = window.Laya;
	        var width = this.maskLayer.width = Laya$1.stage.width;
	        var height = this.maskLayer.height = Laya$1.stage.height;
	        if (this.lockLayer)
	            this.lockLayer.size(width, height);
	        this.maskLayer.graphics.clear(true);
	        this.maskLayer.graphics.drawRect(0, 0, width, height, Laya.UIConfig.popupBgColor);
	        this.maskLayer.alpha = Laya.UIConfig.popupBgAlpha;
	        for (var i = this.numChildren - 1; i > -1; i--) {
	            var item = this.getChildAt(i);
