import { DiagonalMovement } from "../core/DiagonalMovement"
	...
	export class BiAStarFinder
	{
		...
		{
			var cmp:Function = function(nodeA:Node, nodeB:Node):number
			{
				return nodeA.f - nodeB.f;
			};
			var startOpenList:Heap = new Heap(cmp), endOpenList:Heap = new Heap(cmp), startNode:Node = grid.getNodeAt(startX, startY), endNode:Node = grid.getNodeAt(endX, endY), heuristic:Function = this.heuristic, diagonalMovement:number = this.diagonalMovement, weight:number = this.weight, abs:Function = Math.abs, SQRT2:number = Math.SQRT2, node:Node, neighbors:any[], neighbor:Node, i:number, l:number, x:number, y:number, ng:number, BY_START:number = 1, BY_END:number = 2;
