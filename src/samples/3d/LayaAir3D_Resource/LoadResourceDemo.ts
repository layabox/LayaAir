...
export class LoadResourceDemo {
	...
	LoadRes() {
		...
		Scene3D.load("res/threeDimen/scene/TerrainScene/XunLongShi.ls", Handler.create(this, function (scene: Scene3D): void {
			...
			Texture2D.load("res/threeDimen/texture/earth.png", Handler.create(this, function (tex: Texture2D): void {
				//使用纹理
				var earth1: MeshSprite3D = (<MeshSprite3D>scene.addChild(new MeshSprite3D(PrimitiveMesh.createSphere(5, 32, 32))));
	...
	onPreLoadFinish() {
		...
		//激活场景中的子节点
		((<MeshSprite3D>this._scene.getChildByName('Scenes').getChildByName('HeightMap'))).active = false;
		((<MeshSprite3D>this._scene.getChildByName('Scenes').getChildByName('Area'))).active = false;
