...
import { Downloader } from "../../laya/net/Downloader";
...
// iOS 高性能：缓存层路径保持原始（downloadFile 用基类、不重写），只在图片加载边界（super.image）编码本地路径；
// .lani/json 等走 readFile，已在 TbFileSystemAdapter.readFile 边界编码。
MgBrowserAdapter.afterInit = function () {
    if (!_isIOSHighPerf()) return;
    const downloader: any = Loader.downloader;
    if (!downloader) return;
    const _superImage = Downloader.prototype.image;
    downloader.image = function (owner: any, url: string, originalUrl: string, onProgress: any, onComplete: any): void {
        const self = this;
        let skipCache = false;
        if (Browser.onTBMiniGame && (window as any).__NOT_TBMINIGAME__ !== undefined)
            skipCache = (window as any).__NOT_TBMINIGAME__;
        if (skipCache) {
            const u = (url.startsWith("http://") || url.startsWith("https://")) ? url : self.escapeURL(url);
            _superImage.call(self, owner, u, originalUrl, onProgress, onComplete);
            return;
        }
        if ((!url.startsWith("http://") && !url.startsWith("https://")) || !self.cacheManager) {
            _superImage.call(self, owner, self.escapeURL(url), originalUrl, onProgress, onComplete);
            return;
        }
        self.cacheManager.getFile(url).then((cacheFilePath: string) => {
            if (cacheFilePath)
                _superImage.call(self, owner, _encPath(cacheFilePath), originalUrl, onProgress, onComplete);
            else {
                self.downloadFile(url, onProgress, (filePath: string, error: string) => {
                    if (filePath)
                        _superImage.call(self, owner, _encPath(filePath), originalUrl, onProgress, onComplete);
                    else
