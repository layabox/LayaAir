...
export class Material extends Resource implements IClone {
    ...
    checkType(type: ShaderFeatureType): boolean {
        if (this.shader && this.shader.shaderType == ShaderFeatureType.LEGACY_DEFAULT)
            return true;
        let isVaild = this.shader && this.shader.shaderType == type;
        if (!isVaild) {
            console.warn("This Renderer expect Material shader type is " + ShaderFeatureType[type] + ", but the Material shader type is " + ShaderFeatureType[this.shader.shaderType] + ".");
        }
        return isVaild;
    }
