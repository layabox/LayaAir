   ...
   uniform vec2 u_clipMatPos;// 这个是全局的，不用再应用矩阵了。
    ...
    void getVertexInfo(inout vertexInfo info){
         ...
         info.color = vec4(1.0,1.0,1.0,1.0);
            ...
            info.lightUV[0].x = (global.x - u_LightAndShadow2DParam.x) / u_LightAndShadow2DParam.z;
            info.lightUV[0].y = 1.0 - (global.y - u_LightAndShadow2DParam.y) / u_LightAndShadow2DParam.w;
