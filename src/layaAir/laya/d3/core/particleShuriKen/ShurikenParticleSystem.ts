...
export class ShurikenParticleSystem extends GeometryElement implements IClone {
		...
		if (renderMode !== -1 && this.maxParticles > 0) {
			...
			if (renderMode === 4) {
				if (mesh) {
					...
					this._vertexBuffer = new VertexBuffer3D(vbMemorySize, WebGLRenderingContext.DYNAMIC_DRAW);
					...
					this._indexBuffer = new IndexBuffer3D(IndexBuffer3D.INDEXTYPE_USHORT, indexCount, WebGLRenderingContext.STATIC_DRAW);
				...
				this._indexStride = 6;
				this._indexBuffer = new IndexBuffer3D(IndexBuffer3D.INDEXTYPE_USHORT, this._bufferMaxParticles * 6, WebGLRenderingContext.STATIC_DRAW);
	...
	_render(state: RenderContext3D): void {
		...
		if (this._firstActiveElement < this._firstFreeElement) {
			...
			gl.drawElements(WebGLRenderingContext.TRIANGLES, indexCount, WebGLRenderingContext.UNSIGNED_SHORT, 2 * this._firstActiveElement * this._indexStride);
			...
			if (this._firstFreeElement > 0) {
				...
				gl.drawElements(WebGLRenderingContext.TRIANGLES, indexCount, WebGLRenderingContext.UNSIGNED_SHORT, 0);
				Stat.trianglesFaces += indexCount / 3;
				Stat.renderBatches++;
			}
		}
