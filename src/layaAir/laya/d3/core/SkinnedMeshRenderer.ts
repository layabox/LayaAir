		...
		for (var k: number = 0, q: number = boneIndices.length; k < q; k++) {
			...
			if (this._skinnedDataLoopMarks[index] === Stat.loopCount) {
				...
				if (!this._cacheAvatar) {
					Utils3D._mulMatrixArray(this._bones[index].transform.worldMatrix.elements, bindPoses[index].elements, data, k * 16);
				} else {//[兼容代码]
					Utils3D._mulMatrixArray(this._cacheAnimationNode[index].transform.getWorldMatrix(), bindPoses[index].elements, data, k * 16);
				}
