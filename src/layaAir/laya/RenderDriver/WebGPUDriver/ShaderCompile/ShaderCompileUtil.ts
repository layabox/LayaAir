import { ShaderCompileCode, TypeOutData, enumOperator } from "./ShaderCompileCode";
import { ShaderToken } from "./ShaderToken";

export class ShaderCompileUtil {
    static checkDef(node: ShaderToken, _defs: Set<string>) {
    ...
    static toscript(root: ShaderToken, def?: Record<string, boolean>, outData?: TypeOutData) {
        if (null == def) def = {};
        let out = this._parseChilds(root, def);

        if (outData) {
            let st = ShaderCompileCode.compile(out);
    ...
    static checkCondition(st: ShaderToken, def: Record<string, boolean>) {
    ...
    private static _parseChilds(parent: ShaderToken, def?: Record<string, boolean>) {
    ...
    private static _getUniforms(root: ShaderToken, uniforms?: Record<string, { node: ShaderToken, num: number }>) {
