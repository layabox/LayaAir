import { ShaderCompileCode, TypeOut, enumDescribe, enumInOut, enumOperator } from "./ShaderCompileCode";
import { ShaderCompileUtil } from "./ShaderCompileUtil";


export class ShaderToken {
    uniform: Record<string, TypeOut>;
    variable: Set<string>;
    structs: Record<string, ShaderToken>;
    ...
    /**函数类型的话会有这些参数数据 */
    parameter: ShaderToken;

    /**这里是数组参数a[][]这种，支持多维数组 */
    parameterArr: ShaderToken[];

    /**参数数据的数据源 */
    owner: ShaderToken;
    /**ifdef的参数 */
    defParam: string[] | ShaderToken;
    ...
    condition(def: Record<string, boolean>) {
        ...
        if (null != def) {
            if ("#ifdef" == this.name || "#ifndef" == this.name) {
                ...
                if (!(this.defParam instanceof ShaderToken)) {
                    let defParm = this.defParam.join(" ");
                    let parmRoot = ShaderCompileCode.compile(defParm);
                    this.defParam = parmRoot;
                }
                return ShaderCompileUtil.checkCondition(this.defParam, def);
            }
    ...
    addParameterArr(param: ShaderToken, parent?: ShaderToken) {
    ...
    setParameter(param: ShaderToken, parent?: ShaderToken) {
        this.parameter = param;
        param.owner = this;
        if (parent)
            param.parent = parent;
    }
    addBody(body: ShaderToken) {
        body.setParent(this);
    }
    setParent(parent: ShaderToken) {
    ...
    private _parseShaderNode(sn: ShaderToken) {
        let ret = '';
    ...
    private _getParameter(param?: ShaderToken, isFor = false) {
        let ret = '';
        if (null == param) {
    ...
    private _getParameterArr(st: ShaderToken, end = ';') {
        let outStr = '';
        if (null != st.type) {
            outStr += st.type + " ";
        }
