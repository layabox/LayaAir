...
export class Templet extends AnimationTemplet {
    ...
    _parse(texture: Texture, createURL: string, skeletonData: ArrayBuffer) {
        ...
        var tTextureLen: number = tByte.readInt32();
        ...
        var tAniCount: number = tByte.readUint16();
        var tSectionArr: any[];
        ...
        var tBoneLen: number = tByte.readInt16();
        var tBoneDic: any = {};
        var tRootBone: Bone;
        ...
        var tLen: number = tByte.readUint16();
        ...
        var tIkLen: number = tByte.readUint16();
        ...
        var tTfLen: number = tByte.readUint16();
        ...
        var tPathLen: number = tByte.readUint16();
        ...
        var tDeformAniLen: number = tByte.readInt16();
        for (let i = 0; i < tDeformAniLen; i++) {
            var tDeformSkinLen: number = tByte.readUint8();
            var tSkinDic: any = {};
            ...
            for (let f: number = 0; f < tDeformSkinLen; f++) {
                ...
                for (let j = 0; j < tDeformSlotLen; j++) {
                    ...
                    for (let k = 0; k < tDeformSlotDisplayLen; k++) {
                        ...
                        tDeformSlotDisplayData.slotIndex = tDSlotIndex = tByte.readInt16();
                        tDeformSlotDisplayData.attachment = tDAttachment = tByte.readUTFString();
        ...
        var tDrawOrderAniLen: number = tByte.readInt16();
        ...
        var tEventAniLen: number = tByte.readInt16();
        ...
        for (let i = 0; i < tEventAniLen; i++) {
            ...
            for (let j = 0; j < tEventLen; j++) {
                ...
                if (this._isParseAudio) tEventData.audioValue = tByte.readUTFString();
        ...
        var tAttachmentLen: number = tByte.readInt16();
        if (tAttachmentLen > 0) {
        ...
        //创建插槽并绑定到骨骼上
        var tBoneSlotLen: number = tByte.readInt16();
        ...
        var tSkinDataLen: number = tByte.readUint8();
                ...
                for (let k = 0; k < tDisplayDataLen; k++) {
                    ...
                    if (tBoneLen > 0) {
                        ...
                        for (let l = 0; l < tBoneLen; l++) {
                            let tBoneId: number = tByte.readUint16();
        ...
        var tReverse: number = tByte.readUint8();
        if (tReverse == 1) {
