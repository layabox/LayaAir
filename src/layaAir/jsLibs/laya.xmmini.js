window.miMiniGame = function (exports, Laya) {
    ...
    class MiniLoader extends Laya.EventDispatcher {
            ...
            if (!KGMiniAdapter.autoCacheFile) {
                ...
                if (!MiniFileMgr.isLocalNativeFile(url) && !MiniFileMgr.getFileInfo(tempurl)) {
                    if (MiniFileMgr.isNetFile(tempurl)) {
                        item.url = tempurl;
                        MiniFileMgr.downOtherFiles(KGMiniAdapter.safeEncodeURI(tempurl), Laya.Handler.create(MiniLoader, MiniLoader.onDownLoadCallBack, [item, thisLoader]), tempurl);
                    }
                    else {
                        MiniLoader.onDownLoadCallBack(item, thisLoader, 0);
                    }
            ...
            if (Laya.Loader.preLoadedMap[tempurl]) {
                ...
                if (!KGMiniAdapter.AutoCacheDownFile) {
                    ...
                    if (!MiniFileMgr.isLocalNativeFile(url) && !MiniFileMgr.getFileInfo(tempurl)) {
                        if (MiniFileMgr.isNetFile(tempurl)) {
                            item.url = tempurl;
                            MiniFileMgr.downFiles(KGMiniAdapter.safeEncodeURI(tempurl), encoding, new Laya.Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [item, contentType, thisLoader]), tempurl, true);
                        }
                        else {
                            MiniFileMgr.readFile(url, encoding, new Laya.Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [item, contentType, thisLoader]), url);
                        }
        ...
        static onLoadImage(item, thisLoader) {
            ...
            if (!KGMiniAdapter.autoCacheFile) {
                item.url = KGMiniAdapter.safeEncodeURI(url);
                thisLoader._download(item);
            }
            else {
                var tempUrl = Laya.URL.formatURL(url);
                if (!MiniFileMgr.isLocalNativeFile(url) && !MiniFileMgr.getFileInfo(tempUrl)) {
                    if (MiniFileMgr.isNetFile(tempUrl)) {
                        item.url = tempUrl;
                        MiniFileMgr.downOtherFiles(KGMiniAdapter.safeEncodeURI(tempUrl), new Laya.Handler(MiniLoader, MiniLoader.onDownImgCallBack, [item, thisLoader]), tempUrl);
                    }
                    else {
