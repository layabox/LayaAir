window.ttMiniGame = function (exports, Laya) {
    ...
    class MiniLoader extends Laya.EventDispatcher {
            ...
            if (!TTMiniAdapter.autoCacheFile) {
                ...
                if (!MiniFileMgr.isLocalNativeFile(url) && !MiniFileMgr.getFileInfo(tempurl)) {
                    if (MiniFileMgr.isNetFile(tempurl)) {
                        item.url = tempurl;
                        MiniFileMgr.downOtherFiles(TTMiniAdapter.safeEncodeURI(tempurl), Laya.Handler.create(MiniLoader, MiniLoader.onDownLoadCallBack, [item, thisLoader]), tempurl);
                    }
                    else {
                        MiniLoader.onDownLoadCallBack(item, thisLoader, 0);
                    }
        ...
        static onLoadOtherFile(item, thisLoader) {
            ...
            if (Laya.Loader.preLoadedMap[tempurl]) {
                ...
                if (!TTMiniAdapter.AutoCacheDownFile) {
                    ...
                    if (!MiniFileMgr.isLocalNativeFile(url) && !MiniFileMgr.getFileInfo(tempurl)) {
                        if (MiniFileMgr.isNetFile(tempurl)) {
                            MiniFileMgr.downFiles(TTMiniAdapter.safeEncodeURI(tempurl), encoding, new Laya.Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [item, contentType, thisLoader]), tempurl, true);
                        }
                        else {
                            MiniFileMgr.readFile(url, encoding, new Laya.Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [item, contentType, thisLoader]), url);
                        }
        ...
        static onLoadImage(item, thisLoader) {
            ...
            if (!TTMiniAdapter.autoCacheFile) {
                item.url = TTMiniAdapter.safeEncodeURI(url);
                thisLoader._download(item);
            }
            else {
                var tempUrl = Laya.URL.formatURL(url);
                if (!MiniFileMgr.isLocalNativeFile(url) && !MiniFileMgr.getFileInfo(tempUrl)) {
                    if (MiniFileMgr.isNetFile(tempUrl)) {
                        MiniFileMgr.downOtherFiles(TTMiniAdapter.safeEncodeURI(tempUrl), new Laya.Handler(MiniLoader, MiniLoader.onDownImgCallBack, [item, thisLoader]), tempUrl);
                    }
                    else {
