window.vvMiniGame = function (exports, Laya) {
    ...
    class MiniLoader extends Laya.EventDispatcher {
            ...
            if (!VVMiniAdapter.autoCacheFile) {
                ...
                if (!MiniFileMgr.isLocalNativeFile(url) && !MiniFileMgr.getFileInfo(tempurl)) {
                    if (MiniFileMgr.isNetFile(tempurl)) {
                        item.url = tempurl;
                        MiniFileMgr.downOtherFiles(VVMiniAdapter.safeEncodeURI(tempurl), Laya.Handler.create(MiniLoader, MiniLoader.onDownLoadCallBack, [item, thisLoader]), tempurl);
                    }
                    else {
                        MiniLoader.onDownLoadCallBack(item, thisLoader, 0);
                    }
        ...
        static onLoadOtherFile(item, thisLoader) {
            ...
            if (Laya.Loader.preLoadedMap[tempurl]) {
                ...
                if (!VVMiniAdapter.AutoCacheDownFile) {
                    if (MiniFileMgr.isNetFile(tempurl)) {
                        MiniFileMgr.downFiles(VVMiniAdapter.safeEncodeURI(tempurl), encoding, new Laya.Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [item, contentType, thisLoader]), tempurl, true);
                    }
                    else
        ...
        static onLoadImage(item, thisLoader) {
            ...
            if (!VVMiniAdapter.autoCacheFile) {
                item.url = VVMiniAdapter.safeEncodeURI(url);
                thisLoader._download(item);
            }
            else {
                var tempUrl = Laya.URL.formatURL(url);
                if (!MiniFileMgr.isLocalNativeFile(url) && !MiniFileMgr.getFileInfo(tempUrl)) {
                    if (MiniFileMgr.isNetFile(tempUrl)) {
                        MiniFileMgr.downOtherFiles(VVMiniAdapter.safeEncodeURI(tempUrl), new Laya.Handler(MiniLoader, MiniLoader.onDownImgCallBack, [item, thisLoader]), tempUrl);
                    }
                    else {
