window.wxMiniGame = function (exports, Laya) {
    ...
    class MiniFileMgr {
        ...
        static read(filePath, encoding = "utf8", callBack = null, readyUrl = "", isSaveFile = false, fileType = "") {
            ...
            fileUrl = Laya.URL.postFormatURL(fileUrl);
        ...
        static readFile(filePath, encoding = "utf8", callBack = null, readyUrl = "", isSaveFile = false, fileType = "", isAutoClear = true) {
            filePath = Laya.URL.postFormatURL(filePath);
    ...
    class MiniLoader extends Laya.EventDispatcher {
        ...
        download(item) {
            var thisLoader = this;
            MiniLoader._loadResourceFilter(item, thisLoader);
        }
        static _loadResourceFilter(item, thisLoader) {
            ...
            switch (type) {
                ...
                case "htmlimage":
                case "nativeimage":
                    MiniLoader.onLoadImage(item, thisLoader);
                    break;
                case Laya.Loader.SOUND:
                    MiniLoader.onLoadSound(item, thisLoader);
                    break;
                default:
                    MiniLoader.onLoadOtherFile(item, type, thisLoader);
            }
        }
        static onLoadSound(item, thisLoader) {
            var url = item.url;
            if (!MiniAdpter.autoCacheFile) {
                MiniLoader.onLoadSoundCB(url, thisLoader, 0);
            }
            else {
                ...
                if (!MiniFileMgr.isLocalNativeFile(url) && !MiniFileMgr.getFileInfo(tempurl)) {
                    if (MiniFileMgr.isNetFile(tempurl)) {
                        MiniFileMgr.downOtherFiles(tempurl, Laya.Handler.create(MiniLoader, MiniLoader.onLoadSoundCB, [item, thisLoader]), tempurl);
                    }
                    else {
                        MiniLoader.onLoadSoundCB(item, thisLoader, 0);
                    }
                }
                else {
                    MiniLoader.onLoadSoundCB(item, thisLoader, 0);
        ...
        static onLoadSoundCB(item, thisLoader, errorCode, tempFilePath = null) {
            ...
            if (!errorCode) {
                ...
                thisLoader.completeItem(item, sound);
            }
            else {
                thisLoader.completeItem(item, null);
            }
        }
        static onLoadOtherFile(item, contentType, thisLoader) {
            var url = item.url;
            var encoding = MiniAdpter.getUrlEncode(url, contentType);
            var tempurl = Laya.URL.formatURL(url);
            let preloadedContent = Laya.Loader.preLoadedMap[item.url];
            if (preloadedContent)
                thisLoader.completeItem(item, preloadedContent);
            else {
                if (!MiniAdpter.AutoCacheDownFile) {
                    if (MiniFileMgr.isNetFile(tempurl)) {
                        thisLoader._download(item);
                    }
                    else {
                        ...
                        MiniFileMgr.readFile(url, encoding, new Laya.Handler(MiniLoader, MiniLoader.onLoadOtherFileCB, [item, contentType, thisLoader]), url);
                    }
                }
                else {
                    if (!MiniFileMgr.isLocalNativeFile(url) && !MiniFileMgr.getFileInfo(tempurl)) {
                        if (MiniFileMgr.isNetFile(tempurl)) {
                            MiniFileMgr.downFiles(tempurl, encoding, new Laya.Handler(MiniLoader, MiniLoader.onLoadOtherFileCB, [item, contentType, thisLoader]), tempurl, true);
                        }
                        else {
                            MiniFileMgr.readFile(url, encoding, new Laya.Handler(MiniLoader, MiniLoader.onLoadOtherFileCB, [item, contentType, thisLoader]), url);
                        }
                        ...
                        MiniFileMgr.readFile(tempUrl, encoding, new Laya.Handler(MiniLoader, MiniLoader.onLoadOtherFileCB, [item, contentType, thisLoader]), url);
                    }
                }
            }
        }
        static onLoadOtherFileCB(item, type = null, thisLoader = null, errorCode = 0, data = null) {
            var url = item.url;
            if (!errorCode) {
                var tempData;
                ...
                thisLoader.completeItem(item, tempData);
            }
            else if (errorCode == 1) {
                MiniLoader.onLoadOtherFile(item, type, thisLoader);
            }
        }
        static onLoadImage(item, thisLoader) {
            var url = item.url;
            if (MiniAdpter.isZiYu || MiniFileMgr.isLocalNativeFile(url)) {
                if (MiniFileMgr.isLocalNativeZipFile(url)) {
                    item.url = MiniFileMgr.getFileNativePath(url);
                }
                thisLoader._download(item);
                return;
            }
            if (!MiniAdpter.autoCacheFile) {
                ...
                if (!MiniFileMgr.isLocalNativeFile(url) && !MiniFileMgr.getFileInfo(tempUrl)) {
                    if (MiniFileMgr.isNetFile(tempUrl)) {
                        MiniFileMgr.downOtherFiles(tempUrl, new Laya.Handler(MiniLoader, MiniLoader.onDownImgCallBack, [item, thisLoader]), tempUrl);
                    }
                    else {
                        MiniLoader.onCreateImage(item, thisLoader, true);
                    }
                }
                else {
                    MiniLoader.onCreateImage(item, thisLoader, false);
                }
            }
        }
        static onDownImgCallBack(item, thisLoader, errorCode, tempFilePath = "") {
            if (!errorCode)
                MiniLoader.onCreateImage(item, thisLoader, false, tempFilePath);
            else {
                thisLoader.completeItem(item, null);
            }
        }
        static onCreateImage(item, thisLoader, isLocal = false, tempFilePath = "") {
            var sourceUrl = item.url;
            var fileNativeUrl;
            if (MiniAdpter.autoCacheFile) {
                if (!isLocal) {
    ...
    class MiniAdpter {
        ...
        static init(isPosMsg = false, isSon = false) {
            ...
            Laya.Loader.prototype._download = Laya.Loader.prototype.download;
            Laya.Loader.prototype.download = MiniLoader.prototype.download;
            if (!MiniAdpter.window.conch) {
