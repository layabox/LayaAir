window.qgMiniGame = function (exports, Laya) {
    ...
    class MiniLoader extends Laya.EventDispatcher {
        ...
        static _loadResourceFilter(item, thisLoader) {
            ...
            switch (type) {
                ...
                case Laya.Loader.SOUND:
                    ...
                    break;
                default:
                    MiniLoader.onLoadOtherFile(item, thisLoader);
            }
        }
            ...
            if (!QGMiniAdapter.autoCacheFile) {
                ...
                if (!MiniFileMgr.isLocalNativeFile(url) && !MiniFileMgr.getFileInfo(tempurl)) {
                    if (MiniFileMgr.isNetFile(tempurl)) {
                        item.url = tempurl;
                        MiniFileMgr.downOtherFiles(QGMiniAdapter.safeEncodeURI(tempurl), Laya.Handler.create(MiniLoader, MiniLoader.onDownLoadCallBack, [item, thisLoader]), tempurl);
                    }
                    else {
                        MiniLoader.onDownLoadCallBack(item, thisLoader, 0);
                    }
                }
                else {
                    MiniLoader.onDownLoadCallBack(item, thisLoader, 0);
        ...
        static onDownLoadCallBack(item, thisLoader, errorCode, tempFilePath = null) {
            ...
            if (!errorCode) {
                ...
                thisLoader.completeItem(item);
            }
            else {
                thisLoader.completeItem(item, null);
            }
        }
        static onLoadOtherFile(item, thisLoader) {
            var url = item.url;
            ...
            if (Laya.Loader.preLoadedMap[tempurl]) {
                ...
                if (!QGMiniAdapter.AutoCacheDownFile) {
                    if (MiniFileMgr.isNetFile(tempurl)) {
                        item.url = tempurl;
                        MiniFileMgr.downFiles(QGMiniAdapter.safeEncodeURI(tempurl), encoding, new Laya.Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [item, contentType, thisLoader]), tempurl, false);
                    }
                    else
                        MiniFileMgr.readFile(url, encoding, new Laya.Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [item, contentType, thisLoader]), url);
                }
                else {
                    if (!MiniFileMgr.isLocalNativeFile(url) && !MiniFileMgr.getFileInfo(tempurl)) {
                        if (MiniFileMgr.isNetFile(tempurl)) {
                            item.url = tempurl;
                            MiniFileMgr.downFiles(QGMiniAdapter.safeEncodeURI(tempurl), encoding, new Laya.Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [item, contentType, thisLoader]), tempurl, true);
                        }
                        else {
                            MiniFileMgr.readFile(url, encoding, new Laya.Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [item, contentType, thisLoader]), url);
                        }
                        ...
                        MiniFileMgr.readFile(tempUrl, encoding, new Laya.Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [item, contentType, thisLoader]), url);
                    }
                }
            }
        }
        static onReadNativeCallBack(item, type = null, thisLoader = null, errorCode = 0, data = null) {
            var url = item.url;
            if (!errorCode) {
                var tempData;
        ...
        static onLoadImage(item, thisLoader) {
            ...
            if (!QGMiniAdapter.autoCacheFile) {
                item.url = QGMiniAdapter.safeEncodeURI(url);
                thisLoader._download(item);
            }
            else {
                var tempUrl = Laya.URL.formatURL(url);
                if (!MiniFileMgr.isLocalNativeFile(url) && !MiniFileMgr.getFileInfo(tempUrl)) {
                    if (MiniFileMgr.isNetFile(tempUrl)) {
                        item.url = tempUrl;
                        MiniFileMgr.downOtherFiles(QGMiniAdapter.safeEncodeURI(tempUrl), new Laya.Handler(MiniLoader, MiniLoader.onDownImgCallBack, [item, thisLoader]), tempUrl);
                    }
                    else {
                    ...
                    MiniLoader.onCreateImage(item, thisLoader);
                }
            }
        }
