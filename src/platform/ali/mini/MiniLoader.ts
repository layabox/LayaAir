...
import { Event } from "laya/events/Event";
	...
	export class MiniLoader  extends EventDispatcher  {
		...
		_loadHttpRequestWhat(url:string, contentType:string):void {
			var thisLoader:any = this;
			var encoding:string = ALIMiniAdapter.getUrlEncode(url,contentType);
			if (Loader.preLoadedMap[url])
				thisLoader.onLoaded(Loader.preLoadedMap[url]);
			else
			{
				var tempurl:string = URL.formatURL(url);
				if (url.indexOf("http://usr/") == -1&& (tempurl.indexOf("http://") != -1 || tempurl.indexOf("https://") != -1) && !ALIMiniAdapter.AutoCacheDownFile) 
				{
		...
		private static onReadNativeCallBack(url:string, type:string = null, thisLoader:any = null, errorCode:number = 0, data:any = null):void {
			if (!errorCode) {
				...
				//主域向子域派发数据
				if(!ALIMiniAdapter.isZiYu &&ALIMiniAdapter.isPosMsgYu && type  != Loader.BUFFER)
				{
					ALIMiniAdapter.window.wx.postMessage({url:url,data:tempData,isLoad:"filedata"});
				}
