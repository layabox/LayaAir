...
import { Event } from "laya/events/Event";	
	...
	export class MiniInput {
		...
		private static _createInputElement():void {
			...
			QQMiniAdapter.window.qq.onWindowResize && QQMiniAdapter.window.qq.onWindowResize(function(res:any):void {
				// QQMiniAdapter.window.dispatchEvent && window.dispatchEvent("resize");
			});
		...
		private static _onStageResize():void {
		 ...
		 static wxinputFocus(e:any):void {
			...
			QQMiniAdapter.window.qq.showKeyboard({defaultValue: _inputTarget.text, maxLength: _inputTarget.maxChars, multiple: _inputTarget.multiline, confirmHold: true, confirmType: _inputTarget["confirmType"]||'done', success: function(res:any):void {
			}, fail: function(res:any):void {
			}});
			
			QQMiniAdapter.window.qq.onKeyboardConfirm(function(res:any):void {
			...
			QQMiniAdapter.window.qq.onKeyboardInput(function(res:any):void {
				var str:string = res ? res.value : "";
		 ...
		 static hideKeyboard():void {
			QQMiniAdapter.window.qq.offKeyboardConfirm();
			QQMiniAdapter.window.qq.offKeyboardInput();
			QQMiniAdapter.window.qq.hideKeyboard({success: function(res:any):void {
				console.log('隐藏键盘')
			}, fail: function(res:any):void {
