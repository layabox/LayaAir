		 ...
		 */
		 static init(isPosMsg:boolean = false, isSon:boolean = false):void {
			...
			if (!QQMiniAdapter.isZiYu) {
				...
				MiniFileMgr.existDir(MiniFileMgr.fileNativeDir, Handler.create(QQMiniAdapter, QQMiniAdapter.onMkdirCallBack));
			}
			QQMiniAdapter.systemInfo = QQMiniAdapter.window.qq.getSystemInfoSync();
			
			QQMiniAdapter.window.focus = function():void {
			};
			//清空路径设定
			...
			QQMiniAdapter.window.logtime = function(str:string):void {
			};
			QQMiniAdapter.window.alertTimeLog = function(str:string):void {
			};
			QQMiniAdapter.window.resetShareInfo = function():void {
			};
			//适配Context中的to对象
			QQMiniAdapter.window.CanvasRenderingContext2D = function():void {
			};
			...
			QQMiniAdapter.window.document.body.appendChild = function():void {
			};
			//获取手机的设备像素比
			...
			// QQMiniAdapter.EnvConfig.load = Loader.prototype._loadResourceFilter;
		...
		/**@private **/
		 static createElement(type:string):any {
			if (type == "canvas") {
				...
				var node:any = QQMiniAdapter._preCreateElement(type);
