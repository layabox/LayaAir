	...
	export class MiniAdpter {
		 ...
		 */
		 static init(isPosMsg:boolean = false, isSon:boolean = false):void {
			...
			MiniAdpter.window.logtime = function(str:string):void {
			};
			MiniAdpter.window.alertTimeLog = function(str:string):void {
			};
			MiniAdpter.window.resetShareInfo = function():void {
			};
			//适配Context中的to对象
			MiniAdpter.window.CanvasRenderingContext2D = function():void {
			};
			...
			MiniAdpter.window.document.body.appendChild = function():void {
			};
			//获取手机的设备像素比
...
//			MiniLocation.__init__();
		...
		/**@private **/
		 static createElement(type:string):any {
			if (type == "canvas") {
				...
				var node:any = MiniAdpter._preCreateElement(type);
		...
		private static postInfoToContext(url:string, atlaspngUrl:string, atlasJson:any):void {
			...
			if (fileNativeUrl) {
				MiniAdpter.window.wx.postMessage({url: URL.formatURL(url), atlasdata: postData, imgNativeUrl: fileNativeUrl, imgReadyUrl: textureUrl, isLoad: "opendatacontext"});
			} else {
