	...
	export class MiniInput {
		...
		private static _createInputElement():void {
			...
			KGMiniAdapter.window.qg.onWindowResize && KGMiniAdapter.window.qg.onWindowResize(function(res:any):void {
				//KGMiniAdapter.window.dispatchEvent && window.dispatchEvent("resize");
			});
		...
		private static _onStageResize():void {
		 ...
		 static wxinputFocus(e:any):void {
			...
			KGMiniAdapter.window.qg.showKeyboard({defaultValue: _inputTarget.text, maxLength: _inputTarget.maxChars, multiple: _inputTarget.multiline, confirmHold: true, confirmType: _inputTarget["confirmType"]||'done', success: function(res:any):void {
