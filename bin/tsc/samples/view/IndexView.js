...
export class IndexView extends IndexViewUI {
    constructor() {
        super();
        this._comboxBigArr = ['Sprite', '动画', '骨骼动画', '混合模式', '区块地图', '滤镜', '粒子', '音频', '文本', 'UI', '计时器', '缓动', '鼠标交互', '屏幕适配', '网络和格式', '调试', '性能测试', 'IDE'];
        /************************sprite-start***************************/
        this._comboBoxSpriteClsArr = [Sprite_DisplayImage, Sprite_Container, Sprite_RoateAndScale, Sprite_DrawPath, Sprite_MagnifyingGlass, Sprite_DrawShapes, Sprite_Cache, Sprite_NodeControl, Sprite_Pivot, Sprite_SwitchTexture];
        this._comboBoxSpriteArr = ['显示图片', '容器', '旋转缩放', '根据数据绘制路径', '遮罩-放大镜', '绘制各种形状', '缓存为静态图像', '节点控制', '轴中心', '切换纹理'];
        /************************sprite-end***************************/
        /************************Animation-start***************************/
        ...
        /************************Animation-end***************************/
        /************************Skeleton-start***************************/
        this._comboBoxSkeletonClsArr = [Skeleton_MultiTexture, Skeleton_SpineEvent, Skeleton_SpineIkMesh, Skeleton_SpineVine, Skeleton_ChangeSkin];
        this._comboBoxSkeletonArr = ['多纹理', 'Spine事件', '橡胶人', '藤蔓', '换装'];
        /************************Skeleton-end***************************/
        /************************BlendMode-start***************************/
        ...
        /************************Filters-end***************************/
        /************************Particle-start***************************/
        this._comboBoxParticleClsArr = [Particle_T1, Particle_T2, Particle_T3];
        this._comboBoxParticleArr = ['粒子演示1', '粒子演示2', '粒子演示3'];
        /************************Particle-end***************************/
        /************************Sound-start***************************/
        ...
        /************************Sound-end***************************/
        /************************Text-start***************************/
        this._comboBoxTextClsArr = [Text_AutoSize, Text_ComplexStyle, Text_Editable, Text_Overflow, Text_Underline, Text_InputSingleline, Text_InputMultiline, Text_MaxChars, Text_Restrict, Text_Scroll, Text_WordWrap, Text_BitmapFont, Text_HTML];
        this._comboBoxTextArr = ['自动调整文本尺寸', '复杂的文本样式', '禁止编辑', 'Overflow', '下划线', '单行输入', '多行输入', '字数限制', '字符限制', '滚动文本', '自动换行', '位图字体', 'HTML文本'];
        /************************Text-end***************************/
        /************************UI-start***************************/
        this._comboBoxUIClsArr = [UI_Label, UI_Button, UI_RadioGroup, UI_CheckBox, UI_Clip, UI_ColorPicker, UI_ComboBox, UI_Dialog, UI_ScrollBar, UI_Slider, UI_Image, UI_List, UI_ProgressBar, UI_Tab, UI_Input, UI_TextArea, UI_Tree];
        this._comboBoxUIArr = ['Label', 'Button', 'RadioGroup', 'CheckBox', 'Clip', 'ColorPicker', 'ComboBox', 'Dialog', 'ScrollBar', 'Slider', 'Image', 'List', 'ProgressBar', 'Tab', 'Input', 'TextArea', 'Tree'];
        /************************UI-end***************************/
        /************************Timer-start***************************/
        ...
        /************************Tween-end***************************/
        /************************Interaction-start***************************/
        this._comboBoxInteractionClsArr = [Interaction_Hold, Interaction_Drag, Interaction_Rotate, Interaction_Scale, Interaction_Swipe, Interaction_CustomEvent, Interaction_Mouse, Interaction_FixInteractiveRegion];
        this._comboBoxInteractionArr = ['Hold', '拖动', '双指旋转（多点触控）', '双指缩放（多点触控）', '滑动', '自定义事件', '鼠标交互', '修正交互区域'];
        /************************Interaction-end***************************/
        /************************SmartScale-start***************************/
        this._comboBoxSmartScaleClsArr = [SmartScale_Scale_EXTRACT_FIT, SmartScale_Scale_NOBORDER, SmartScale_Scale_SHOW_ALL, SmartScale_T];
        this._comboBoxSmartScaleArr = ['缩放-Extract Fit', '缩放-No Border', '缩放-Show All', '屏幕适配'];
        /************************SmartScale-end***************************/
        /************************Network-start***************************/
        this._comboBoxNetworkClsArr = [Network_POST, Network_GET, Network_XML, Network_Socket, Network_ProtocolBuffer];
        this._comboBoxNetworkArr = ['POST', 'GET', 'XML', 'Websocket', 'ProtocolBuffer'];
        /************************Network-end***************************/
        /************************Debug-start***************************/
        ...
        this._comboBoxPerformanceTestClsArr = [PerformanceTest_Maggots, PerformanceTest_Cartoon, PerformanceTest_Cartoon2, PerformanceTest_Skeleton];
        this._comboBoxPerformanceTestArr = ['虫子(慎入)', '卡通人物', '卡通人物2', '骨骼'];
        /************************PerformanceTest-end***************************/
        /************************IDE-start***************************/
        this._comboBoxIDEClsArr = [IDE_Project];
        this._comboBoxIDEArr = ['显示IDE创建的界面'];
        /************************IDE-end***************************/
        this._bigIndex = 0;
        this.initView();
        this.initEvent();
    ...
    onBigComBoxSelectHandler(index) {
        ...
        this.smallComBox.selectedIndex = 0;
        this.smallComBox.visibleNum = labelStr.split(",").length;
    }
    onRightBtnClick() {
    }
    onLeftBtnClick() {
