...
export class Shader_GlowingEdge {
    ...
    //初始化shader
    initShader() {
        var attributeMap = { 'a_Position': VertexMesh.MESH_POSITION0, 'a_Normal': VertexMesh.MESH_NORMAL0, 'a_Texcoord': VertexMesh.MESH_TEXTURECOORDINATE0, 'a_BoneWeights': VertexMesh.MESH_BLENDWEIGHT0, 'a_BoneIndices': VertexMesh.MESH_BLENDINDICES0 };
        var uniformMap = { 'u_Bones': Shader3D.PERIOD_CUSTOM, 'u_CameraPos': Shader3D.PERIOD_CAMERA, 'u_MvpMatrix': Shader3D.PERIOD_SPRITE, 'u_WorldMat': Shader3D.PERIOD_SPRITE, 'u_texture': Shader3D.PERIOD_MATERIAL, 'u_marginalColor': Shader3D.PERIOD_MATERIAL, 'u_DirectionLight.Direction': Shader3D.PERIOD_SCENE, 'u_DirectionLight.Color': Shader3D.PERIOD_SCENE };
        //创建自定义shader
        var customShader = Shader3D.add("CustomShader");
        //为当前自定义的shader添加SubShader
        var subShader = new SubShader(attributeMap, uniformMap, SkinnedMeshSprite3D.shaderDefines);
