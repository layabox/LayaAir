...
var screenShotDir = path.join(__dirname,'../../data/screenshots');
//记录错误
let errPath =path.join(__dirname,'error'); 
...
puppeteer.launch({
    ...
    async browser => {
        ...
        var testid=0;
        let errors=[]
        let casesToTest = args.length > 0 ? args : fs.readdirSync('../tsc/test/cases/2d');
        for await(const onecase of casesToTest){
            ...
            console.log('start [',testid,']',onecase,'http://127.0.0.1:4000/test.html?'+js);
            testid++;
            await page.goto('http://127.0.0.1:4000/test.html?'+js);
